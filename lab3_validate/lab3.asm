...
USART1_RX1_INTERRUPT:
	...
	STATE_0:
		CPI R16, 'S'          ; Compares and branches to INPUT_ERROR if R16 != 'S'
		BRNE INPUT_ERROR
		JMP END_STATE_CHECK
	...
	STATE_1:
		...
		SUBI R16, '0'
		CPI R16, 3		     ; Compares and branches to INPUT_ERROR if R16 > 2 or R16 < 0
		BRSH INPUT_ERROR
		JMP END_STATE_CHECK
	; State corresponding to angle sign {-, +}
	STATE_2:
		MOV R20, R16          ; Copies USART1's input R16 to R20
		CPI R16, '+'          ; Compares and branches to CHECK_MINUS if R16 != '+'
		BRNE CHECK_MINUS
		JMP END_STATE_CHECK
		CHECK_MINUS:
		CPI R16, '-'          ; Compares and branches to INPUT_ERROR if R16 != '-'
		BRNE INPUT_ERROR
		JMP END_STATE_CHECK
	; State corresponding to first angle digit input {0, ..., 9}
	STATE_3:
		SUBI R16, '0'         ; Transforms input from ascii to binary
		CPI R16, 10		      ; Compares and branches to INPUT_ERROR if R16 > 9 or R16 < 0
		BRSH INPUT_ERROR
...
INPUT_ERROR:
	LDI ZH, HIGH(2*ERROR)   
	LDI ZL, LOW(2*ERROR)
	CALL PRINT_USART1       ; Prints error to USART1
	LDI R17, 5              ; Loads finished state
	POP R16
	RETI
...
ERROR:
	.DB RETURN, LINEFEED, "Use correct format SXsAA!", RETURN, LINEFEED, "SXsAA Format: S={S}, X={0,1,2}, s={-,+}, A={0->9}" , RETURN, LINEFEED, '$'
NEWLINE:
	.DB RETURN, LINEFEED, '$'
